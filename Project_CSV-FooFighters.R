x = read.csv2("FooFighters.csv",stringsAsFactors=FALSE,sep=",");
x = sapply(x, as.character)x = data.matrix(x)x = apply(x, 2, as.numeric) #changing the data type from character to numeric

new_data = scale(x[,3:11]) #Standardizing the datasetView(new_data)new_x = cbind(x[,1:2],new_data) #Binding the columns togetherView(new_x)
...
cat("Most difference is", max(diff), "\n")cat("Least difference is", min(diff), "\n")plot(diff, main = "Effect on Popularity", ylab = "Difference between old and new sse", xlab = "Attributes")